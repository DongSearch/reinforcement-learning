...
class DQN:
    ...
    def learn(self, timesteps):
        ...
        for timestep in range(1, timesteps + 1):
            ...
            # depending on epsilon, we do exploration or exploitation and then return action
            a = self.predict(s, epsilon)
            # we get other value of variables from taking action we chose
            s_, r, terminated, _, _ = self.env.step(a)
            # store to the buffer
            self.replay_buffer.put(s, a, r, s_, terminated)
            # update s
            s = s_
            # reward is also updated
            episode_rewards.append(r)
            # if it is terminated or enough rewardsm, we reset
            if terminated or len(episode_rewards) >= 500:
                s, _ = self.env.reset()
                all_rewards.append(sum(episode_rewards))
                episode_rewards = []

            # if lengh of buffersize is greather than batchsize, we do optimization
    ...
    def predict(self, s, epsilon=0.0):
        ...
        if np.random.uniform(0, 1) > epsilon:
            ...
            q_value = self.net(torch.FloatTensor(s).unsqueeze(0))
    ...
    def compute_msbe_loss(self):
        ...
        states, actions, rewards, states_, terminated = self.replay_buffer.get(self.batch_size)
        ...
        # extract the Q-value for each action actually taken, for each sample in the batch
        # now it is our prediction
        ...
        if self.use_target_network:
            ...
            q_values_ = self.target_net(states_)
        else:
            q_values_ = self.net(states_)
        q_values_ = q_values_.max(dim=1)[0]

        # TODO 1.5: Compute update target
        # The target we want to update our network towards
        target_q_values = torch.Tensor(rewards) + self.gamma * (1.0 - torch.Tensor(terminated)) * q_values_
...
def epsilon_by_timestep(timestep, epsilon_start=1.0, epsilon_final=0.01, frames_decay=10000):
    ...
    # at the beginning, we want to explore more(e is high), but as times go by, we want to exploit
    # what it has learnd with small e
